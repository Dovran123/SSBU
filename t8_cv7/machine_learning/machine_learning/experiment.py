...
class Experiment:
    ...
    def __init__(self, models, models_params, additional_metrics=None, n_replications=100):
        """
        ...
        - additional_metrics: dict, a dictionary of additional metrics to be evaluated.
    ...
    def train_and_evaluate_model(self, model_name, X_resampled, y_resampled, datascaler, replication):
        ...
        # Calculate additional metrics if specified
        additional_metric_values = {}
        if self.additional_metrics:
            for metric_name, metric_func in self.additional_metrics.items():
                additional_metric_values[metric_name] = metric_func(y_test, predictions)

        self.store_results(model_name, replication, accuracy, f1, roc_auc, best_params, **additional_metric_values)
    ...
    def store_results(self, model_name, replication, accuracy, f1, roc_auc, best_params, **additional_metrics):
        ...
        for metric_name, metric_value in additional_metrics.items():
            new_row[metric_name] = metric_value

        self.results = pd.concat([self.results, new_row], ignore_index=True)
        logging.info(
            f"{model_name}, Replication: {replication}, Accuracy: {accuracy:.4f}, F1: {f1:.4f}, "
            f"ROC AUC: {roc_auc:.4f},  Params: {best_params}, {', '.join([f'{name}: {value:.4f}' for name, value in additional_metrics.items()])}")
